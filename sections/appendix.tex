...
This section shows examples of using \LaTeX for common operations. 
...
Notice how the ``label'' command has been used several times in the source code for this section. This command allows you to mark an element, be it a section, figure, etc. to make a numeric reference to it. To reference a ``label'', use the ``ref'' command including the name of the reference:

This is the section \ref{sec:background}.
...
This saves us from having to directly write the indexes of the sections and figures we want to mention, since LaTeX does it for us and also takes care of keeping them updated in case they change (try moving this section to the end of the document and see how all referenced indices are updated automatically). Also, ``ref'' references act as hyperlinks within the document that take you to the referenced element when you click on them.
...
\begin{lstlisting}[language=Java, caption={Java code}, label={cod:java}, captionpos=b]
public class Test {
